...
export const getSecrets = async (filters?: {
  ...
  if (filters?.status && filters.status !== 'all') {
    filteredSecrets = filteredSecrets.filter(
      secret => secret.status.toLowerCase() === filters.status?.toLowerCase()
    );
  }
...
export const getSecretById = async (
  id: string
): Promise<ApiResponse<Secret | null>> => {
...
export const getSecretStats = async (): Promise<
  ApiResponse<{
    total: number;
    active: number;
    expired: number;
...
> => {
  ...
  return createSuccessResponse(
    stats,
    'Secret statistics retrieved successfully'
  );
};
