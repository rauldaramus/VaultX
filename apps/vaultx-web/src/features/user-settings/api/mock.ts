...
export const getAccountInfo = async (): Promise<ApiResponse<AccountInfo>> => {
  ...
  return createSuccessResponse(mockAccountInfo, "Account information retrieved successfully")
}

export const updateAccountInfo = async (info: Partial<AccountInfo>): Promise<ApiResponse<AccountInfo>> => {
  ...
  if (info.email && info.email !== mockAccountInfo.email && info.email === "taken@example.com") {
    return createErrorResponse("Email already in use", 409, [
      { field: "email", message: "This email address is already in use", code: "EMAIL_TAKEN" }
    ])
  }
  
  mockAccountInfo = { ...mockAccountInfo, ...info }
  return createSuccessResponse(mockAccountInfo, "Account information updated successfully")
}
...
export const changePassword = async (data: {
  ...
  if (data.newPassword.length < 8) {
    return createErrorResponse("Password too weak", 400, [
      { field: "newPassword", message: "Password must be at least 8 characters long", code: "WEAK_PASSWORD" }
    ])
  }
...
export const deleteAccount = async (data: {
  ...
  if (data.password !== "password123") {
    return createErrorResponse("Password is incorrect", 400, [
      { field: "password", message: "Password is incorrect", code: "INVALID_PASSWORD" }
    ])
  }
  ...
  if (data.confirmation !== "DELETE") {
    return createErrorResponse("Confirmation text is incorrect", 400, [
      { field: "confirmation", message: "Please type DELETE to confirm", code: "INVALID_CONFIRMATION" }
    ])
  }
...
export const getSecuritySettings = async (): Promise<ApiResponse<SecuritySettings>> => {
  ...
  return createSuccessResponse(mockSecuritySettings, "Security settings retrieved successfully")
}

export const updateSecuritySettings = async (settings: Partial<SecuritySettings>): Promise<ApiResponse<SecuritySettings>> => {
  await simulateDelay(800)
  
  mockSecuritySettings = { ...mockSecuritySettings, ...settings }
  return createSuccessResponse(mockSecuritySettings, "Security settings updated successfully")
}
...
export const revokeSession = async (sessionId: string): Promise<ApiResponse<{ message: string }>> => {
  await simulateDelay(600)
  
  if (sessionId === "session_1") {
    return createErrorResponse("Cannot revoke current session", 400, [
      { field: "sessionId", message: "You cannot revoke your current session", code: "CURRENT_SESSION" }
    ])
  }
