...
import { ArrowUp, ArrowDown, Lock, Eye, Clock, ShieldCheck } from "lucide-react"
...
export function StatsCards() {
  ...
  const statsData = [
    { 
      ...
      change: stats?.trends.secretsCreatedChange ? `${stats.trends.secretsCreatedChange > 0 ? '+' : ''}${stats.trends.secretsCreatedChange}%` : null,
      icon: Lock 
    },
    { 
      title: "Secrets Viewed", 
      value: stats?.secretsViewed?.toString() || "0", 
      change: stats?.trends.secretsViewedChange ? `${stats.trends.secretsViewedChange > 0 ? '+' : ''}${stats.trends.secretsViewedChange}%` : null,
      icon: Eye 
    },
    { 
      title: "Active Secrets", 
      value: stats?.activeSecrets?.toString() || "0", 
      change: stats?.trends.activeSecretsChange ? `${stats.trends.activeSecretsChange > 0 ? '+' : ''}${stats.trends.activeSecretsChange}%` : null,
      icon: Clock 
    },
    { 
      title: "Current Plan", 
      ...
      {statsData.map((stat, index) => (
        ...
        >
          ...
          <CardContent className="group">
            <div className="text-2xl font-bold transition-all duration-300 hover:scale-105">{stat.value}</div>
            {stat.change ? (
              <p className="text-xs text-gray-400 flex items-center transition-all duration-200 hover:text-gray-300">
                ...
                <span
                  className={`transition-colors duration-200 ${stat.change.startsWith("+") ? "text-green-500 hover:text-green-400" : "text-red-500 hover:text-red-400"}`}
                >
                  {stat.change}
                </span>
                &nbsp;from last month
