...
export function ActivityChart() {
  ...
  const data = stats?.chartData.secretActivity.map(item => ({
    name: item.day,
    total: item.secrets
  })) || []
  return (
    <Card className="bg-gray-900 border-gray-800 hover-lift animate-fade-in-up opacity-0 animate-stagger-2">
      <CardHeader className="transition-all duration-300 hover:bg-gray-800/30">
        <CardTitle className="transition-colors duration-200 hover:text-gray-200">Secret Activity</CardTitle>
      ...
      <CardContent className="pl-2 transition-all duration-300">
        <div className="animate-scale-in opacity-0" style={{ animationDelay: "0.5s" }}>
          {loading ? (
