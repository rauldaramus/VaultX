import type { ApiToken, ApiUsageStats, CreateTokenRequest } from "@vaultx/shared"
...
export const getApiToken = async (id: string): Promise<ApiResponse<ApiToken | null>> => {
...
export const createApiToken = async (data: CreateTokenRequest): Promise<ApiResponse<ApiToken>> => {
  await simulateDelay(1000)
  
  // Validate name
  if (mockTokens.some(t => t.name === data.name)) {
    return createErrorResponse("Token name already exists", 409, [
      { field: "name", message: "A token with this name already exists", code: "DUPLICATE_NAME" }
    ])
  }
  ...
  const newToken: ApiToken = {
    id: `token_${Date.now()}`,
    name: data.name,
    token: `vx_${data.environment || 'live'}_pk_${Math.random().toString(36).substr(2, 32)}`,
...
export const updateApiToken = async (id: string, data: Partial<ApiToken>): Promise<ApiResponse<ApiToken | null>> => {
...
export const deleteApiToken = async (id: string): Promise<ApiResponse<boolean>> => {
  await simulateDelay(600)
...
export const getApiUsage = async (period?: string): Promise<ApiResponse<ApiUsageStats>> => {
  ...
  return createSuccessResponse(stats, "API usage statistics retrieved successfully")
}

export const regenerateApiToken = async (id: string): Promise<ApiResponse<ApiToken | null>> => {
  ...
  const newTokenString = `vx_${token.token.includes('test') ? 'test' : 'live'}_pk_${Math.random().toString(36).substr(2, 32)}`
  ...
  mockTokens[tokenIndex] = updatedToken
  return createSuccessResponse(updatedToken, "API token regenerated successfully")
}
