...
interface ApiData {
  ...
  updateToken: (id: string, data: Partial<ApiToken>) => Promise<ApiToken | null>
...
export const useApiData = (): ApiData => {
  ...
  const createToken = useCallback(async (data: CreateTokenRequest): Promise<ApiToken | null> => {
    try {
      setError(null)
  ...
  const updateToken = useCallback(async (id: string, data: Partial<ApiToken>): Promise<ApiToken | null> => {
    try {
      setError(null)
  ...
  const regenerateToken = useCallback(async (id: string): Promise<ApiToken | null> => {
    try {
      setError(null)
