        ...
        <CardContent className="space-y-4">
          {currentSession ? (
            <div className="relative p-4 border border-border rounded-lg bg-card">
              <Badge className="absolute top-3 right-3 bg-green-500/20 text-green-300">Current</Badge>
              <p className="text-sm text-muted-foreground mb-2">Started {timeAgo(currentSession.lastActiveAt)}</p>
              ...
              <div className="flex items-center gap-2 text-sm mt-1">
                ...
                <span>Location: Madrid, Spain</span> {/* Mocked location for now */}
              </div>
            </div>
          ) : (
            <p className="text-muted-foreground text-center">Could not load current session.</p>
          )}
          ...
          {otherSessions.length > 0 ? (
            <div className="space-y-4">
              ...
              {otherSessions.map((session) => (
                <div key={session.id} className="relative p-4 border border-border rounded-lg bg-card">
                  <p className="text-sm text-muted-foreground mb-2">Started {timeAgo(session.lastActiveAt)}</p>
                  <div className="flex items-center gap-2 text-sm">
                    <Laptop className="h-4 w-4 text-muted-foreground" />
                    <span>
                      Device: {session.deviceInfo.os} • {session.deviceInfo.browser} {session.deviceInfo.device}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-sm mt-1">
                    <Wifi className="h-4 w-4 text-muted-foreground" />
            ...
            <p className="text-muted-foreground text-center">No other active sessions on your account.</p>
          )}
        </CardContent>
      </Card>
    </div>
