/**
 * @file: password.service.spec.ts
 * @version: 0.0.0
 * @author: Raul Daramus
 * @date: 2025
 * Copyright (C) 2025 VaultX by Raul Daramus
 *
 * This work is licensed under the Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License.
 * To view a copy of this license, visit http://creativecommons.org/licenses/by-nc-sa/4.0/
 * or send a letter to Creative Commons, PO Box 1866, Mountain View, CA 94042, USA.
 *
 * You are free to:
 *   - Share — copy and redistribute the material in any medium or format
 *   - Adapt — remix, transform, and build upon the material
 *
 * Under the following terms:
 *   - Attribution — You must give appropriate credit, provide a link to the license,
 *     and indicate if changes were made.
 *   - NonCommercial — You may not use the material for commercial purposes.
 *   - ShareAlike — If you remix, transform, or build upon the material, you must
 *     distribute your contributions under the same license as the original.
 */

import type { ConfigService } from '@nestjs/config';

import type { AppConfig } from '../../config';

import { PasswordService } from './password.service';

const createConfigService = (rounds: number) =>
  ({
    get: jest.fn().mockReturnValue({
      auth: {
        bcryptSaltRounds: rounds,
      },
    } as AppConfig),
  } as unknown as ConfigService<AppConfig>);

describe('PasswordService', () => {
  it('should hash and verify passwords using configured rounds', () => {
    const configService = createConfigService(12);
    const service = new PasswordService(configService);

    const password = 'Str0ng!Password';
    const hash = service.hash(password);

    expect(hash).toBeDefined();
    expect(hash).not.toEqual(password);
    expect(service.verify(password, hash)).toBe(true);
    expect(service.verify('wrong-password', hash)).toBe(false);
  });

  it('should produce different hashes for the same password', () => {
    const configService = createConfigService(10);
    const service = new PasswordService(configService);

    const password = 'Another$ecret123';
    const hashOne = service.hash(password);
    const hashTwo = service.hash(password);

    expect(hashOne).not.toEqual(hashTwo);
    expect(service.verify(password, hashOne)).toBe(true);
    expect(service.verify(password, hashTwo)).toBe(true);
  });
});
